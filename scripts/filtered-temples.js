...
function addTemplesToHtml(temple) {
...
function createTempleCard(filter = null) {
  if (filter == "large") {
    ...
    result.forEach(addTemplesToHtml);
    ...
    const result = temples.filter((temple) => {
      const year = parseInt(temple.dedicated.split(",")[0]); // Extract the year
      return year > 2000;
    });
    ...
    temples.forEach(addTemplesToHtml);
  }
}